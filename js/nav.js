 ...
 * Gemini: changed 2026-09-03 - Added News navigation link (GH Issue #20) agent change
 * Gemini: changed 2026-09-03 - Added Static Site Search via Pagefind modal & shortcuts (GH Issue #17) agent change
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const searchSvg = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>`;

    // Auto-inject Search stylesheet if missing
    // Gemini: changed 2026-09-08 - Bump version parameter for mobile search input fix (GH Issue #53) agent change
    // Gemini: changed 2026-09-05 - Bump version parameter for mobile search modal clipping fix (GH Issue #38) agent change
    // Gemini: changed 2026-09-03 - Bump version timestamp for search styling updates agent change
    if (!document.getElementById('ebo-search-css')) {
        const searchCss = document.createElement('link');
        searchCss.id = 'ebo-search-css';
        searchCss.rel = 'stylesheet';
        searchCss.href = `${prefix}css/search.css?v=20260908160000`;
    ...
    // Auto-inject Pagefind Search Dialog Modal
    // Gemini: changed 2026-09-08 - Pre-render fallback search form with input field so mobile modal is never empty (GH Issue #53) agent change
    // Gemini: changed 2026-09-03 - Removed ESC text badge leaving clean X close button agent change
    if (!document.getElementById('ebo-search-modal')) {
        const searchModalHTML = `
        <dialog id="ebo-search-modal">
            ...
            <div class="search-modal-body">
                <div id="pagefind-modal-container">
                    <form class="pagefind-ui__form ebo-search-fallback-form" action="${prefix}search.html" method="get">
                        <input class="pagefind-ui__search-input" type="search" name="q" placeholder="Search site..." autocomplete="off" autocorrect="off" autocapitalize="off" enterkeyhint="search" aria-label="Search site">
                        <button class="pagefind-ui__search-clear" type="button" aria-label="Clear search" style="display: none;"></button>
                    </form>
                </div>
            </div>
        </dialog>
    ...
    <nav>
        ...
        <div class="nav-links" id="nav-links">
            ...
            <!-- Gemini: added 2026-09-03 - Added News navigation link between Trips and Resources (GH Issue #20) agent change -->
            <a href="${prefix}news.cards.html">News</a>
            <a href="${prefix}resources.cards.html">Resources</a>
            <a href="${prefix}join.cards.html">Join</a>
            <div class="nav-auth-container">
                <button class="nav-icon-link nav-search-button" id="nav-search-btn" type="button" aria-label="Search site" title="Search site (Press / or Cmd+K)">
                    ${searchSvg}
                </button>
                <span class="auth-separator">|</span>
                ${getAuthBtnHTML()}
    ...
    // Gemini: created 2026-09-03 (GH Issue #17) agent change
    // ---------------------------------------------------------
    const searchBtn = document.getElementById('nav-search-btn');
    const searchModal = document.getElementById('ebo-search-modal');
    let pagefindLoadingPromise = null;
    let pagefindInitialized = false;

    /**
     * Wires up interaction for the pre-rendered fallback search input clear button.
     * Gemini: added 2026-09-08 - Fallback input clear interaction (GH Issue #53) agent change
     */
    const setupFallbackClear = () => {
        const container = document.getElementById('pagefind-modal-container');
        if (!container) return;
        const fallbackInput = container.querySelector('.ebo-search-fallback-form .pagefind-ui__search-input');
        const fallbackClear = container.querySelector('.ebo-search-fallback-form .pagefind-ui__search-clear');
        if (fallbackInput && fallbackClear) {
            fallbackInput.addEventListener('input', () => {
                fallbackClear.style.display = fallbackInput.value ? 'inline-flex' : 'none';
            });
            fallbackClear.addEventListener('click', (e) => {
                e.preventDefault();
                fallbackInput.value = '';
                fallbackClear.style.display = 'none';
                fallbackInput.focus();
            });
        }
    };
    ...
    /**
     * Asynchronously ensures Pagefind UI assets are loaded and mounts Pagefind into #pagefind-modal-container.
     * Preserves and triggers any query typed into the initial fallback input.
     * Gemini: added 2026-09-08 - Background preloading & seamless fallback replacement (GH Issue #53) agent change
     * @returns {Promise<void>} Resolves when Pagefind is initialized or fallback is retained.
     */
    const initPagefindWidget = async () => {
        if (pagefindInitialized) return;
        if (pagefindLoadingPromise) return pagefindLoadingPromise;

        pagefindLoadingPromise = (async () => {
            const pagefindBase = prefix ? `${prefix}pagefind/` : '/pagefind/';

            // Ensure Pagefind UI CSS is loaded
            if (!document.getElementById('pagefind-ui-css')) {
                const pfCss = document.createElement('link');
                pfCss.id = 'pagefind-ui-css';
                pfCss.rel = 'stylesheet';
                pfCss.href = `${pagefindBase}pagefind-ui.css`;
                document.head.appendChild(pfCss);
            }

            // Lazy-load pagefind-ui.js
            if (!window.PagefindUI) {
                await new Promise((resolve) => {
                    ...
                    script.onerror = () => {
                        console.warn('Pagefind script failed to load, retaining fallback search form');
                        resolve();
                    };
            ...
            // Mount Pagefind UI widget and swap out fallback form seamlessly
            if (window.PagefindUI) {
                const container = document.getElementById('pagefind-modal-container');
                if (container && !container.querySelector('.pagefind-ui')) {
                    const fallbackInput = container.querySelector('.pagefind-ui__search-input');
                    const typedVal = fallbackInput ? fallbackInput.value : '';
                    const isInputFocused = fallbackInput && (document.activeElement === fallbackInput);

                    container.innerHTML = '';
                    const pf = new window.PagefindUI({
                        element: '#pagefind-modal-container',
                    ...
                    if (typedVal) {
                        pf.triggerSearch(typedVal);
                    }

                    // Maintain focus if modal is open or fallback had focus
                    const newSearchInput = container.querySelector('.pagefind-ui__search-input');
                    if (newSearchInput && (isInputFocused || (searchModal && searchModal.open))) {
    ...
    /**
     * Preloads Pagefind assets during browser idle time to avoid latency when modal opens.
     * Gemini: added 2026-09-08 - Preload during idle time (GH Issue #53) agent change
     */
    if (typeof window.requestIdleCallback === 'function') {
        window.requestIdleCallback(() => { initPagefindWidget(); }, { timeout: 3500 });
    } else {
        setTimeout(initPagefindWidget, 1500);
    }

    /**
     * Opens the global search modal, closes mobile nav, and ensures search input is focused immediately.
     * Gemini: changed 2026-09-08 - Auto-close mobile nav and synchronously focus input for mobile Safari (GH Issue #53) agent change
     */
    window.openSearchModal = () => {
        ...
        // Synchronously focus the search input so iOS Safari / mobile browser displays keyboard
        const searchInput = searchModal.querySelector('.pagefind-ui__search-input');
        if (searchInput) {
            searchInput.focus();
        }

        // Ensure Pagefind UI widget is loaded and mounted
        initPagefindWidget();
    };
    ...
    window.addEventListener('keydown', (e) => {
        ...
        // Shortcut: '/' to search when not typing
        if (e.key === '/' && !isEditing) {
            e.preventDefault();
            window.openSearchModal();
        }
