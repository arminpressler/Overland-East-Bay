...
class EBORsvpModule {
    ...
    constructor(container, tripId) {
        ...
        // Gemini: added 2026-09-16 - Support convoy mode and Overland Bound rally point routing (GH Issue #82) agent change
        this.mode = this.container ? (this.container.dataset.mode || 'standard') : 'standard';
        this.obRallyPointUrl = this.container ? (this.container.dataset.obRallyPointUrl || null) : null;
    ...
    /**
     * Constructs URL to Group Trip Agreement page with contextual tripId and capacity params.
     * Gemini: changed 2026-09-14 - Support /archive/ subdirectory path for relative links (GH Issue #77) agent change
    ...
    getAgreementPageUrl() {
        ...
        const prefix = (isEventPage || isArchivePage) ? '../' : '';
     ...
     * Gemini: changed 2026-09-16 - Support convoy mode title in loading state (GH Issue #82) agent change
     */
    renderLoading() {
        const title = this.mode === 'convoy' ? '🚙 Convoy Organization (East Bay)' : '⛺ Trip RSVP';
        this.container.innerHTML = `
            <div class="ebo-rsvp-module-container">
                <div class="ebo-rsvp-header">
                    <h3 class="ebo-rsvp-title">${title}</h3>
     ...
     * Gemini: changed 2026-09-16 - Added convoy mode labels, dynamic OB rally point bridge box, and instructions (GH Issue #82) agent change
     * @param {boolean} isLoggedIn - User authentication state.
     */
    renderModule(isLoggedIn) {
        const isConvoy = this.mode === 'convoy';
        const rsvpPath = this.getParticipantsPageUrl();

        // Inject dynamic Overland Bound sign-up box if obRallyPointUrl configured and not already present in DOM
        if (this.obRallyPointUrl && !document.querySelector('.ebo-ob-signup-box') && this.container.parentNode) {
            const obBox = document.createElement('div');
            obBox.className = 'ebo-rsvp-module-container ebo-ob-signup-box';
            obBox.innerHTML = `
                <div class="ebo-rsvp-header">
                    <div>
                        <h3 class="ebo-rsvp-title">🧭 Official Sign-Up → Overland Bound Rally Point</h3>
                        ...
                        <div style="margin-top: 0.65rem; color: #cbd5e1; font-size: 0.88rem; line-height: 1.5;">
                            Official event capacity, project assignments, and volunteer rosters are managed on Overland Bound (Rally Point #3319).
                        </div>
                    </div>
                    <div>
                        <a href="${this.obRallyPointUrl}" target="_blank" rel="noopener noreferrer" class="ebo-rsvp-btn ebo-rsvp-btn-ob">
                            <span>🏕️ Sign Up on Overland Bound (Rally Point #3319) ↗</span>
                        </a>
                    </div>
                </div>
            `;
            this.container.parentNode.insertBefore(obBox, this.container);
        }
        ...
        if (!isLoggedIn) {
            buttonHtml = `
                <button class="ebo-rsvp-btn ebo-rsvp-btn-auth" id="ebo-rsvp-main-btn">
                    <span>🔐 ${isConvoy ? 'Sign-in to Join Convoy' : 'Sign-in to RSVP'}</span>
                </button>
            `;
            ...
            buttonHtml = `
                <button class="ebo-rsvp-btn ebo-rsvp-btn-action" id="ebo-rsvp-main-btn" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid #4ade80;">
                    <span>✓ ${isConvoy ? 'In Convoy (Change RSVP)' : 'Going (Change RSVP)'}</span>
                </button>
            `;
            ...
            buttonHtml = `
                <button class="ebo-rsvp-btn ebo-rsvp-btn-action" id="ebo-rsvp-main-btn" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid #f87171;">
                    <span>❌ ${isConvoy ? 'Not in Convoy (Change RSVP)' : 'Not Going (Change RSVP)'}</span>
                </button>
            `;
        } else if (this.isFull) {
            buttonHtml = `
            ...
            buttonHtml = `
                <button class="ebo-rsvp-btn ebo-rsvp-btn-action" id="ebo-rsvp-main-btn">
                    <span>${isConvoy ? '🚙 RSVP for Convoy' : '✍️ RSVP Here'}</span>
                </button>
            `;
        }
        ...
        const countPillsHtml = `
            <div style="display: inline-flex; gap: 0.5rem; align-items: center; margin-top: 0.5rem; flex-wrap: wrap;">
                <span style="background: rgba(43, 147, 72, 0.15); color: #4ade80; border: 1px solid rgba(74, 222, 128, 0.3); padding: 0.2rem 0.65rem; border-radius: 12px; font-size: 0.85rem; font-weight: 600;">
                    ✓ ${this.goingCount}${capacityStr} ${isConvoy ? 'in Convoy' : 'Going'}
                </span>
                ${this.waitlistCount > 0 ? `
                ...
                ${this.notGoingCount > 0 ? `
                    <span style="background: rgba(239, 68, 68, 0.15); color: #f87171; border: 1px solid rgba(248, 113, 113, 0.3); padding: 0.2rem 0.65rem; border-radius: 12px; font-size: 0.85rem; font-weight: 600;">
                        ❌ ${this.notGoingCount} ${isConvoy ? 'Not in Convoy' : 'Not Going'}
                    </span>
                ` : ''}
            </div>
        `;
        ...
        const convoyCalloutHtml = isConvoy ? `
            <div class="ebo-convoy-callout">
                <strong>🚗 Convoy Organization:</strong> This RSVP is exclusively for coordinating the East Bay group driving out to the event together. To officially participate in the Trail Guardian project, make sure to sign up on the Overland Bound Rally Point above.
            </div>
        ` : '';

        const agreementPath = this.getAgreementPageUrl();
        const titleText = isConvoy ? '🚙 Convoy Organization (East Bay)' : '⛺ Trip RSVP & Roster';
        const subtitleText = isConvoy
            ? 'Use this RSVP to coordinate the East Bay convoy out to the event.'
            : 'Join the team for this upcoming adventure! View participant list or update your status.';
        const rosterLinkText = isConvoy ? '📋 View Convoy Roster →' : '📋 View Participant List →';

        this.container.innerHTML = `
            <div class="ebo-rsvp-module-container">
                <div class="ebo-rsvp-header">
                    <div>
                        <h3 class="ebo-rsvp-title">${titleText}</h3>
                        <p style="color: #94a3b8; margin: 0.3rem 0 0 0; font-size: 0.95rem;">
                            ${subtitleText}
                        </p>
                        ${countPillsHtml}
    ...
    showStatusChangeModal() {
        ...
        const isConvoy = this.mode === 'convoy';
        const currentStatusLabel = isCurrentlyGoing ? (isConvoy ? '✓ In Convoy' : '✓ Going') : (isConvoy ? '❌ Not in Convoy' : '❌ Not Going');
        const goingBtnLabel = isConvoy ? '✓ In Convoy' : '✓ Going';
        const notGoingBtnLabel = isConvoy ? '❌ Not in Convoy' : '❌ No Longer Going';
            ...
            <div class="ebo-modal-backdrop" id="ebo-rsvp-status-modal">
                <div class="ebo-modal-card" style="max-width: 480px;">
                    <div class="ebo-modal-header">
                        <h3>🔄 Update Your ${isConvoy ? 'Convoy' : 'RSVP'} Status</h3>
