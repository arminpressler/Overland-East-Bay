...
function renderTripContextBanner() {
    ...
    // Gemini: changed 2026-09-14 - Resolve trip page whether in /events/ or /archive/ (GH Issue #77) agent change
    let tripPageUrl = `/events/${context.tripId}.html`;
    if (document.referrer && document.referrer.includes('/archive/')) {
        tripPageUrl = `/archive/${context.tripId}.html`;
    } else {
        fetch(`/events/${context.tripId}.html`, { method: 'HEAD' }).then(res => {
            if (!res.ok && res.status === 404) {
                const backLink = bannerContainer.querySelector('a.btn-secondary');
                if (backLink) backLink.href = `/archive/${context.tripId}.html`;
            }
        }).catch(() => {});
    }
